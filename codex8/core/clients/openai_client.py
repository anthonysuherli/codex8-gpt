"""Direct OpenAI client — structured-output calls.

    codex8 pipeline → AsyncOpenAI → OpenAI API

The exploration pipeline's LLM calls (query planning, finding extraction) use
the `openai` SDK directly. Models are OpenAI API model IDs, e.g.
``gpt-5.6-terra`` or ``gpt-5.6-luna``. Pydantic models are the
...
def client() -> AsyncOpenAI:
    """Memoized AsyncOpenAI pointed at the OpenAI API."""
    s = get_settings()
...
async def text_completion(
    *,
    ...
    """One plain-text completion through OpenAI — no structured output.

    Mirrors ``structured_completion``'s client setup but omits any
...
async def structured_completion(
    *,
    ...
    Calls OpenAI directly. On any failure (transport error, refusal, or
    ...
    strict ``json_schema`` response format — correct for fully-typed schemas.
    Set ``False`` for schemas with free-form ``dict[str, Any]`` fields (e.g.
    the extractor's ``content``): strict mode forces those to ``{}``, so we
    instead instruct the schema in the prompt and validate the reply.
...
async def _attempt(
    ...
    """Get structured output. With ``use_json_schema`` try the strict
...
async def _parse_json_schema(
    ...
    """OpenAI structured output via `response_format: {type: json_schema}`.

    The OpenAI SDK returns the JSON as a *string* in `message.content` (not the
    SDK's `.parsed` field), so we validate it against the model ourselves.
    """
    completion = await client().chat.completions.create(
        model=model,
        messages=[
...
async def _parse_prompt_json(
    ...
    validate. Covers models that reject the json_schema parameter."""
