...
def _default_db_path() -> str:
    """``CODEX8_DB_PATH`` if set, else ``~/.codex8/codex8.db`` (dir created)."""
    env = os.environ.get("CODEX8_DB_PATH")
    if env:
        return env
    try:
        from codex8.core.config import get_settings

        configured_path = get_settings().codex8_db_path
    except Exception:  # noqa: BLE001 — local store stays usable without Settings
        configured_path = None
    if configured_path:
        return configured_path
    home = Path.home() / ".codex8"
    home.mkdir(parents=True, exist_ok=True)
    return str(home / "codex8.db")
...
class SQLiteStore:
    ...
    def set_drift_marker(self, kb_id: str, count: int) -> None:
        ...
        Called once per offer (by ``codex8_mark_drift_offered``) so the next session
